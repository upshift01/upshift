...
const AdminCalendar = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/admin/bookings?start_date=${startDate}&end_date=${endDate.toISOString().split('T')[0]}`,
  ...
  const handleConfirmBooking = async (bookingId) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/admin/bookings/${bookingId}/confirm`,
        { 
          method: 'POST',
          headers: getAuthHeader() 
        }
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      setActionLoading(bookingId);
      const response = await fetch(
        `${API_URL}/api/admin/bookings/${bookingId}/cancel`,
        { 
          method: 'POST',
          headers: getAuthHeader() 
        }
  ...
  const formatDateTime = (dateStr, timeStr) => {
    const date = new Date(dateStr);
    return `${date.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })} at ${timeStr}`;
  };
  ...
  const isSlotAvailable = (date, time) => {
    ...
    return daySlots.slots.some(s => s.time === time && s.available);
  };
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">Manage strategic call bookings</p>
        </div>
                    ...
                    <div className="grid grid-cols-8 border-b">
                      ...
                      {weekDays.map((day, idx) => (
                        ...
                        >
                          ...
                          <p className={`text-lg font-semibold ${day.toDateString() === new Date().toDateString() ? 'text-blue-600' : ''}`}>
                            {day.getDate()}
                          </p>
                        </div>
                      ))}
                            ...
                            return (
                              ...
                              >
                                {booking ? (
                                  ...
                                  <div className={`h-full ${isAvailable ? 'bg-green-50' : 'bg-gray-50'}`} />
                                )}
                              </div>
                            );
                          })}
                    ...
                    filteredBookings.sort((a, b) => new Date(a.date) - new Date(b.date)).map((booking) => (
                      <div
                        ...
                        className={`p-4 border rounded-lg cursor-pointer transition-all hover:shadow-md ${
                          selectedBooking?.id === booking.id ? 'border-blue-500 bg-blue-50' : ''
                        }`}
                        onClick={() => setSelectedBooking(booking)}
                      >
