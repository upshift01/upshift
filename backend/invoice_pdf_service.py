...
class InvoicePDFGenerator:
    ...
    def generate_customer_invoice_pdf(self, invoice, reseller=None, reseller_settings=None):
        ...
        # HEADER - Two separate tables side by side
        left_data = [
            [Paragraph(f'<b><font size="16">{company}</font></b>', styles['Normal'])],
            [Paragraph(f'<font size="9" color="gray">{contact.get("email", "")}</font>', styles['Normal'])],
            [Paragraph(f'<font size="9" color="gray">{contact.get("phone", "")}</font>', styles['Normal'])],
        ]
        if seller_vat:
            left_data.append([Paragraph(f'<font size="9" color="gray">VAT: {seller_vat}</font>', styles['Normal'])])
        
        right_data = [
            [Paragraph('<b><font size="20">INVOICE</font></b>', styles['Normal'])],
            [Paragraph(f'<font size="10" color="gray">{invoice.get("invoice_number", "")}</font>', styles['Normal'])],
            [Paragraph(f'<font size="9">Date: {self._fmt_date(invoice.get("created_at"))}</font>', styles['Normal'])],
        ]
        
        left_table = Table(left_data, colWidths=[8*cm])
    ...
    def generate_reseller_invoice_pdf(self, invoice, reseller=None, platform_settings=None):
        ...
        # HEADER
        left_data = [
            [Paragraph(f'<b><font size="16">{platform}</font></b>', styles['Normal'])],
            [Paragraph(f'<font size="9" color="gray">{contact.get("email", "")}</font>', styles['Normal'])],
            [Paragraph(f'<font size="9" color="gray">{contact.get("phone", "")}</font>', styles['Normal'])],
        ]
        if seller_vat:
        ...
        right_data = [
            [Paragraph('<b><font size="20">TAX INVOICE</font></b>', styles['Normal'])],
            [Paragraph(f'<font size="10" color="gray">{invoice.get("invoice_number", "")}</font>', styles['Normal'])],
            [Paragraph(f'<font size="9">Date: {self._fmt_date(invoice.get("created_at"))}</font>', styles['Normal'])],
        ]
